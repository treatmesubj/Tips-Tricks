...
/* Fail Query if duplicates */
WITH cnum_recs AS (
  SELECT SK_EMPLOYEE_CNUM_HISTORICAL, COUNT(*) AS CNT
  FROM EPM.DIM_EMPLOYEE
  GROUP BY SK_EMPLOYEE_CNUM_HISTORICAL
  HAVING COUNT(*) > 1
),
dupe_check AS (
  SELECT
...
SELECT CREATOR, NAME, TYPE, REMARKS, BASE_NAME, BASE_SCHEMA, TBSPACE
FROM SYSIBM.SYSTABLES
WHERE NAME LIKE 'FACT_HEAD_COUNT_MONTHLY_STATIC%'
  AND TYPE IN ('T', 'V', 'A')
  AND CREATOR NOT IN ('SYSIBM')
...
ORDER BY TABSCHEMA, TABNAME, TYPE
WITH UR;


-- dummy table
CREATE TABLE SCHEMA.DUMMY AS (
  SELECT *
  FROM SYSIBM.SYSDUMMY1
)
WITH DATA
IN DTS_TABLESPACE ORGANIZE BY COLUMN;

-- dummy view
CREATE OR REPLACE VIEW SCHEMA_VIEW.DUMMY
AS
SELECT *
FROM SCHEMA.DUMMY:
...
WITH DATA
IN TS_MYTABLESPACE ORGANIZE BY COLUMN;
...
CREATE ALIAS SCHEMA.ALIAS FOR SCHEMA.TABLE;
/* DB2 SQL Error: SQLCODE=-551, SQLSTATE=42501, SQLERRMC=USR_ID;REPLACE ALIAS;SCHEMA.TABLE */
TRANSFER OWNERSHIP OF ALIAS SCHEMA.TABLE TO USER USR_ID PRESERVE PRIVILEGES;
...
/* Count Distinct */
SELECT YEAR, QUARTER, MONTH, GUY, COUNT(DISTINCT ROLE) ROLES
FROM SCHEMA.TABLE
GROUP BY YEAR, QUARTER, MONTH, GUY
HAVING ROLES > 1
ORDER BY ROLES DESC
